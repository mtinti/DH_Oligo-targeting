...
def get_field(cell, key):
    ...
    #print(cell_dict)
    return cell_dict.get(key,'none').replace('%2','')
...
def add_track(bed_file = str, chro = str, 
    ...
    if bed_file.endswith('_d.bed') or '_d_' in bed_file:
        df.columns = ['chr', 'base', 'coverage']
    if bed_file.endswith('_bg.bed') or '_bg_' in bed_file:
...
if __name__ == '__main__':
    ...
    counts = pd.read_table(sys.argv[9], sep='\t', index_col=[0], comment='#')
    print(counts.head())
    counts = counts[counts[counts.columns[6]]>0]
    counts = counts[counts[counts.columns[6]] >counts[counts.columns[6]].mean() + counts[counts.columns[6]].std()]
