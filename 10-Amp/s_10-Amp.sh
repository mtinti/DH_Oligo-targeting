...
#$ -pe smp 8
#$ -N s_10-Amp
#$ -o /cluster/majf_lab/mtinti/simone_amplicon/TRYlhwR/Clean/10-Amp/s_10-Amp_$JOB_ID
...
primerF='TGCTGTGAAAAAGTTGCATGA'
primerR='CAGGCGGGCGATAGTTAAG'
...
echo 'run bowtie2'
(bowtie2 --very-sensitive-local --rg-id '10-Amp' --rg SM:10-Amp \
-p 8 -x $path_genome_index -1 $out_1 -2 $out_2) \
...
#echo 'run picard MarkDuplicates' crash out of memory
...
##freebayes -f $path_genome_index'.fasta' $path_out$base_fastq'sorted.bam' | vcffilter -f "QUAL > 20" > \
##$path_out$base_fastq'results.vcf'
##gzip $path_out$base_fastq'results.vcf'
...
echo 'run alfred'
alfred qc -r $path_genome_index'.fasta' -j $path_out'alfred/'$base_fastq'qc.json.gz' -o $path_out'alfred/'$base_fastq'qc.tsv.gz' $path_out$base_fastq'sorted.bam'
...
echo 'run 7'
bedtools genomecov \
-ibam $path_out$base_fastq'sorted.bam' \
-bg -pc > $path_out$base_fastq'_coverage_bg.bed'


echo 'run 7.0'
bedtools genomecov \
-ibam $path_out$base_fastq'sorted.bam' \
-d -pc > $path_out$base_fastq'_coverage_d.bed'

echo 'run 7.1'
python mylib/plot_chr_coverage.py \
$path_out$base_fastq'_coverage_d.bed' $path_out$base_fastq'coverage_d.png'


python mylib/extract_barcodes_def2.py \
$path_out$base_fastq'sorted.bam' $primerF $primerR 2

echo 'run sort'
samtools sort -@ 8 -o $path_out$base_fastq'sorted_F.bam' $path_out$base_fastq'sorted_F.bam'
samtools sort -@ 8 -o $path_out$base_fastq'sorted_R.bam' $path_out$base_fastq'sorted_R.bam'
samtools sort -@ 8 -o $path_out$base_fastq'sorted_FR.bam' $path_out$base_fastq'sorted_FR.bam'
samtools sort -@ 8 -o $path_out$base_fastq'sorted_RR.bam' $path_out$base_fastq'sorted_RR.bam'
 
echo 'run indexes'
samtools index $path_out$base_fastq'sorted_F.bam'
samtools index $path_out$base_fastq'sorted_R.bam'
samtools index $path_out$base_fastq'sorted_FR.bam'
samtools index $path_out$base_fastq'sorted_RR.bam'


#see notes for naming files
echo 'run genomecov'
bedtools genomecov -ibam $path_out$base_fastq'sorted_F.bam' \
-bg > $path_out$base_fastq'_coverage_bg_pc_F.bed'
bedtools genomecov -ibam $path_out$base_fastq'sorted_R.bam' \
-bg > $path_out$base_fastq'_coverage_bg_pc_R.bed'
bedtools genomecov -ibam $path_out$base_fastq'sorted_FR.bam' \
-bg > $path_out$base_fastq'_coverage_bg_pc_FR.bed'
bedtools genomecov -ibam $path_out$base_fastq'sorted_RR.bam' \
-bg > $path_out$base_fastq'_coverage_bg_pc_RR.bed'

echo 'run genomecov2'
bedtools genomecov -ibam $path_out$base_fastq'sorted_F.bam' \
-d -pc > $path_out$base_fastq'_coverage_d_F.bed'
bedtools genomecov -ibam $path_out$base_fastq'sorted_R.bam' \
-d -pc > $path_out$base_fastq'_coverage_d_R.bed'
bedtools genomecov -ibam $path_out$base_fastq'sorted_FR.bam' \
-d -pc > $path_out$base_fastq'_coverage_d_FR.bed'
bedtools genomecov -ibam $path_out$base_fastq'sorted_RR.bam' \
-d -pc > $path_out$base_fastq'_coverage_d_RR.bed'
...
--max-gap 100 \
-q 0.01 \
--broad --outdir $path_out'macs2_'$base_fastq

echo 'run featureCounts1' 
...
$path_out$base_fastq'sorted_F.bam' \
$path_out$base_fastq'sorted_R.bam' \
$path_out$base_fastq'sorted_FR.bam' \
$path_out$base_fastq'sorted_RR.bam'





echo 'run plot_region_coverage'

python mylib/plot_region_coverage.py $gff_file \
$path_out'macs2_'$base_fastq'/NA_peaks.xls' \
$path_out$base_fastq'_coverage_d.bed' \
$path_out$base_fastq'_coverage_d_F.bed' \
$path_out$base_fastq'_coverage_d_R.bed' \
$path_out$base_fastq'_coverage_d_FR.bed' \
$path_out$base_fastq'_coverage_d_RR.bed' \
$path_out'/' \
$path_out'counts.txt' \
$experiment
...
echo 'run 10'
cp templates/assemble_template3.ipynb $TMPDIR
sed -i "s@{_EXPERIMENT}@$experiment@g" $TMPDIR'/assemble_template3.ipynb'
sed -i "s@{_FASTQ_HEADER}@$base_fastq@g" $TMPDIR'/assemble_template3.ipynb'
sed -i "s@{genome}@$genome@g" $TMPDIR'/assemble_template3.ipynb'

mkdir $TMPDIR'/mylib'
cp mylib/plot_region_coverage.py  $TMPDIR'/mylib'

jupyter nbconvert --to notebook --execute $TMPDIR'/assemble_template3.ipynb'  --ExecutePreprocessor.timeout=600
rm $TMPDIR'/assemble_template3.ipynb'

jupyter nbconvert --output-dir=$TMPDIR --to html $TMPDIR'/assemble_template3.nbconvert.ipynb'
...
cp templates/coverage_template3.ipynb $TMPDIR
sed -i "s@{_EXPERIMENT}@$experiment@g" $TMPDIR'/coverage_template3.ipynb'
sed -i "s@{_FASTQ_HEADER}@$base_fastq@g" $TMPDIR'/coverage_template3.ipynb'
sed -i "s@{genome}@$genome@g" $TMPDIR'/coverage_template3.ipynb'



jupyter nbconvert --to notebook --execute $TMPDIR'/coverage_template3.ipynb'  --ExecutePreprocessor.timeout=600
rm $TMPDIR'/coverage_template3.ipynb'

jupyter nbconvert --output-dir=$TMPDIR --to html $TMPDIR'/coverage_template3.nbconvert.ipynb'



rm $path_out$base_fastq'_coverage_d'*
rm $path_out$base_fastq'_coverage_bg_pc_'*
rm $path_out$base_fastq'sorted_F'*
rm $path_out$base_fastq'sorted_R'*
rm $path_out$base_fastq'sorted_WO'*
